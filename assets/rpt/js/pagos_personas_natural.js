$(function(){
	...
	showPagoTotalPersonaNatural();

	function showListPagoEmpresas(){
		$.ajax({
			type: 'ajax',
			method: 'get',
			url: baseUrl + "RptController/showListPagosPersonasNatural",
			async: false,
			dataType: 'json',
	...
	function showPagoTotalPersonaNatural(){
		$.ajax({
			type: 'ajax',
			method: 'get',
			url: baseUrl + "RptController/sumaTotalPagosPersonasNaturales",
			async: false,
			dataType: 'json',
			success: function(data){
					$('#totalpgspn').text('Total: S/.'+data.total);
				},
				error:function(){
					swal("¡Ups!", "Algo salió mal!. Refresca la página", "error");
				}
