$(function(){
	...
	$('#showdata').on('click', '.btnView', function(){
		...
		$.ajax({
			type: 'ajax',
			method: 'get',
			url: baseUrl + "emp/CursoController/viewCurso",
			data: {id: id},
			async: false,
			dataType: 'json',
		...
		$.ajax({
			...
			error:function(){
				bootbox.alert('Error al ver detalle');
			}
		});
	}
		);
	...
	$('#showdata').on('click', '.btnReservar', function(){
		...
		$.ajax({
			...
			success: function(data){
				...
				$('#lblfchinicio').text(data.FechaInicio);
				//$('textarea[name=txtdescripcion]').val(data.descripcion);
				$('.txtdescripspn').html(data.descripcion);
			...
			error: function(){
				 bootbox.alert('Error al Reservar');
			}
		});
	...
	$('#btnSaveReserva').click(function(){
		...
		var ct_total = $('select[name=txtcostotal]');
		var resultad = '';
		...
		if(ct_total.val()<1){
			alert("El precio total no puede se 0");
		}else{
			$('#dslscupos').removeClass('has-error');
			resultad +='4';
		}
			...
			if(resultad=='1234'){
				...
				error: function(){
					bootbox.alert('Error al guaradr la reserva');
				}
			});
		    }
	});
   ...
   //Eliminar Reserva
	$('#showdatareservas').on('click', '.btnDeletRes', function (){
		...
		bootbox.confirm("Desea cancelar la reserva?", function(result){
			if(Boolean(result)){
				$.ajax({
				type: 'ajax',
				method: 'get',
	...
	$('#btnpgTotal').click(function(){
		...
		if(pagotota.val()<1){
			...
			$('#modalpagoreserva').modal({backdrop: 'static',});
			$('#modalpagoreserva').find('.tit-modal').text('Pago');
			$('#formPagoReserva').attr('action', baseUrl + 'emp/CursoController/savePago');
		}
	});
	...
	$('#btnSavePagoReserva').click(function(){
		...
		if(id_banco.val()==''){
			$('#dnombanco').addClass('has-error');
		}else{
			$('#dnombanco').removeClass('has-error');
			resultad +='1';
		}
		...
		if(codigoperacion.val()==''){
			$('#dcoperacion').addClass('has-error');
		}else{
		...
		if(pagotota.val()<1){
			...
			if(resultad=='1234'){
				...
				error: function(){
					bootbox.alert('Error al realizar el pago');
				}
			});
		    }
	});
	...
	//Boton Generar Grupo
	$('#showdatapagados').on('click', '.btnInscribir', function(){
		var idRpagado = $(this).attr('data');
		$('#modalgenerargrupo').modal('show');
		$('#modalgenerargrupo').find('.tit-modal').text('Grupo para el Curso');
		$('#formGenerarGrupo').attr('action', baseUrl + 'emp/GrupoController/saveGrupoForEmpresa');
		...
		$.ajax({
			...
			dataType: 'json',
			success: function(data){
				$('input[name=txtnombrecurso]').val(data.nombrecurso);
				$('input[name=txtnombregrupo]').val(data.nombrecurso);
				$('input[name=txtpgidcurso]').val(data.id_curso);
				$('input[name=txtidReserva]').val(data.id_reserva);
			},
			error: function(){
				 bootbox.alert('Error obtener detalle 2');
			}
		});
	});
	//.............
	...
	$('#btnSaveGrup').click(function(){
		...
		var getIdRes = $('#txtidReserva').val();

		//validar
		var nombregrup = $('input[name=txtnombregrupo]');
		var ridCurso = $('input[name=txtpgidcurso]');
		var resultad = '';
			...
			if(resultad=='12'){
				...
				success: function(respuesta){
					...
					$('#formGenerarGrupo')[0].reset();
						...
						dnotificacion.showNotification('top','right');
						setTimeout("window.location.href=baseUrl+'emp/e_grupos';",1000);
				},
				error: function(){
					alert('Error 2');
				}
	...
	$('select#idslecurso').on('change',function(){
		...
		if(nomcur !=''){
 			$.ajax({
				type: 'ajax',
				method: 'get',
				url: baseUrl + "emp/CursoController/showAllCurso",
				data: {nomcur: nomcur},
				async: false,
				dataType: 'json',
				success: function(data){
				...
				for (i=0; i<data.length; i++){
					...
					html +='<tr>'+
								...
								'<td class="td-actions text-right">'+
									'<a href="javascript:;" type="button" title="Ver Detalle" class="btn btn-info btnView" data="'+data[i].id_curso+'"><i class="material-icons">visibility</i></a> '+btnreserva+
								'</td>'+
							'</tr>';
				}
				$('#showdata').html(html);
	...
	function showAllCurso(){
		var txtc='Todo';
		var url1=baseUrl + "emp/CursoController/showAllCurso";
		$.ajax({
				type: 'ajax',
				method: 'get',
			url: url1,
			...
			error:function(){
				bootbox.alert('Error al listar los cursos disponibles');
			}
		});
	}
	...
	function showAllReservas(){
		var url=baseUrl + "emp/CursoController/showAllReserva";
		$.ajax({
			type: 'ajax',
			method: 'get',
			url: url,
	...
	//Mostrar Lista Pagados
	function showAllPagado(){
		var url=baseUrl + "emp/CursoController/showAllPagado";
		$.ajax({
			type: 'ajax',
			method: 'get',
			url: url,
			...
			success: function(data){
				...
				$('#showdatapagados').html(html);
			},
			error:function(){
				bootbox.alert('Error en Listar Pagados');
			}
		});
	}
	...
	function showAllCursoFiltro(getfdesde,getfhasta){
		...
		$.ajax({
			type:'get',
			method: 'get',
			url: baseUrl + 'emp/cursoController/showAllCursoForDate',
			...
			error:function(){
				bootbox.alert('Error al listar los cursos disponibles 2');
			}
		});
	}
